import { h, link, arrow, eyebrow, segmented, says } from "../ui.js";
...
export default function Accounts({ params }) {
  ...
  const render = () => {
    ...
    if (roi) rows = rows.filter(a => (a.ev / a.cost) * 100 / 10 >= roi); // ev per cost scaled as a rough ROI proxy
    ...
    count.textContent = `${rows.length} accounts${roi ? ` with expected ROI above ${roi}%` : ""}`;
  ...
  return h("div", {},
    h("section", { class: "reveal" }, eyebrow("Accounts"), title, sub, vocab),
    ...
    h("section", { class: "section reveal" }, link("/learn/expected-value", h("span", { class: "link" }, "Learn: expected value ", arrow()))),
  );
}
