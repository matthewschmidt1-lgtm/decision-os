...
export default function Accounts({ params }) {
  ...
  const tbody = h("tbody");
  let showAll = false;
  const more = h("button", { type: "button", class: "btn btn-ghost", style: { marginTop: "16px" }, onClick: () => { showAll = !showAll; render(); } });
  ...
  const render = () => {
    ...
    more.hidden = rows.length <= 20; more.textContent = showAll ? "Show top 20" : `Show all ${rows.length} accounts`;
    tbody.replaceChildren(...visible.map((a, i) => h("tr", { class: "clickable", tabindex: "0", onClick: () => navigate(`/accounts/${a.id}`), onKeydown: e => { if (e.key === "Enter") navigate(`/accounts/${a.id}`); } },
      ...
      h("td", { class: "num" }, `${Math.round(a.probability * 100)}%`), h("td", { class: "num" }, money(a.value)), h("td", { class: `num ${a.ev < 0 ? "bad" : ""}`, style: { fontWeight: 600 } }, money(a.ev)))));
    count.textContent = `${showAll || rows.length <= 20 ? rows.length : `Top 20 of ${rows.length}`} accounts${roi ? ` where expected value is more than ${roi}% of the prize` : ""}${rows.some(r => r.ev < 0) ? `. ${rows.filter(r => r.ev < 0).length} have negative expected value: they cost more to pursue than they're likely to return.` : ""}`;
  ...
  return h("div", {},
    ...
    h("section", { style: { marginTop: "20px" }, class: "stack reveal" }, line, count,
      h("div", { class: "table-wrap" }, h("table", { class: "table" }, h("thead", {}, h("tr", {}, h("th", {}, "#"), h("th", {}, "Account"), h("th", { class: "num" }, "Velocity"), h("th", { class: "num" }, "Volume"), h("th", { class: "num" }, "Margin Δ"), h("th", { class: "num" }, "P(win)"), h("th", { class: "num" }, "Value"), h("th", { class: "num" }, "Expected value"))), tbody)), more),
