...
| `js/models.js` | Pure decision models: marginal analysis, utility ranking, UCB hour allocation, Bayes, expected value, decision tree, value of information, fingerprint, trade-budget allocation (`tradeGain`, `allocateBudget`, `quadrant`), whole-budget rebalancing (`rebalance`, `rebalanceRange`), `money`/`pct` formatting |
...
| `js/scenarios.js` | 33 practice scenarios, tracks, 5-minute challenge ids, skills, levels |
...
- **Rebalance** (`/portfolio#rebalance`) keeps the total fixed and moves money until every brand's next dollar returns the same amount, subject to a cut limit. "Let it shrink" stops every brand at its $1 point, which must match the brand popup's trim and room figures (a test checks this). A move is "Robust" only if it goes the same way by at least $10K at both ends of the brand's `r0` range; otherwise "Test first".
